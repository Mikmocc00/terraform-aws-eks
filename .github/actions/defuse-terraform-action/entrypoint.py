...
# Inizializza l'estrattore Terraform
tf_extractor = TerraformMetricsExtractor(path_to_repo='.', clone_repo_to='.', at='release') if language == 'terraform' else None

for file in files:
    # Se stiamo analizzando Terraform, ignoriamo i file che non sono .tf
    if language == 'terraform' and not file.filename.endswith('.tf'):
        continue
